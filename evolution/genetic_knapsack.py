...
def generate_initital_population(population_size: int, individual_size: int):
    # Population could be randomly initialized individuals instead
    population = [mutate_individual([0] * individual_size)] * population_size
    return population
...
def mutate_individual(individual: list[int]):
    mutation_index = rng.integers(len(individual))
...
def generate_new_population(
    ...
    children = [mutate_individual(child) for child in children]
...
def run_ga(
    ...
    population = generate_initital_population(population_size, len(knapsack_items))
    ...
    for i in range(number_of_generations):
        population = generate_new_population(
            population, survival_rate, knapsack_items, knapsack_capacity
        )
...
if __name__ == "__main__":
    ...
    result = run_ga(
        population_size, survival_rate, number_of_generations, knapsack_items, knapsack_capacity
    )
    print(result)
